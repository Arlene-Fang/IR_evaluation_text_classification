...
def set_features(filename):
    ...
    wfile = open('feats.' + classname + '.improve2', 'w')
    for line in file:
        if not line.split():
            continue
        arr = line.lower()
        ...
        tweet = re.sub(r'\d', ',', tweet)  # remove numbers
        #tweet = re.sub(r'\W+', ',', tweet)  # remove tokenise
        tweet_list = re.findall('[a-zA-Z\#][0-9a-zA-Z\_]*', tweet)
        #print(tweet_list)
        tweet_list_hash = []
        cache = {}
        tweet_list_stem = []
        for t in tweet_list:
            if t.startswith('#'):
                tweet_list_hash.append(re.sub('#', '', t))
                tweet_list_hash.append(re.sub('#', '', t))
            else:
                tweet_list_hash.append(t)
        #print(tweet_list_hash)
        #tweet_list = list(tweet.split(","))
        #print(tweet_list)
        while '' in tweet_list_hash:
            tweet_list_hash.remove('')
        while 'â' in tweet_list_hash:
            tweet_list_hash.remove('â')
